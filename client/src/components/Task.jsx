...
const Task = ({ task, onEditTask, onDeleteTask, onToggleCompleted }) => {
  ...
  const [currentISTDateTime, setCurrentISTDateTime] = useState("");
  ...
  useEffect(() => {
    ...
    const updateCurrentTime = () => {
      setCurrentISTDateTime(getCurrentISTDateTime());
    };

    // Set an interval to update the current time every minute
    const interval = setInterval(updateCurrentTime, 60000);

    // Initialize currentISTDateTime with the initial value
    updateCurrentTime();
  ...
  return (
    <li className="mb-1 border-b border-gray-300 space-y-2">
      ...
      <div className="text-lg text-gray-500">{currentISTDateTime}</div>
    </li>
  );
};
